...
def new_style(klass):
    ...
    """
    return type(
        "NewStyleClass<{}>".format(klass.__name__), (klass, object), {}
    )
...
class PythonModuleFinder(new_style(distutils_build_py)):
    ...
    def __init__(
        self, packages, package_dir, py_modules, alternative_build_base=None
    ):
        """
    ...
    def find_package_modules(self, package, package_dir):
        ...
        if (
            ...
            package_dir = os.path.join(
                self.alternative_build_base, package_dir
            )
        ...
        def _strip_directory(entry):
            module_file = entry[2]
            if (
                self.alternative_build_base is not None
                and module_file.startswith(self.alternative_build_base)
            ):
                module_file = module_file[
                    len(self.alternative_build_base) + 1 :
                ]
