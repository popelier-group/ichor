...
class _ExtensionsGenericMeta(GenericMeta):
    def __subclasscheck__(self, subclass):
        ...
        """
        if sys.version_info[:3] >= (3, 5, 3) or sys.version_info[:3] < (
            3,
            5,
            0,
        ):
            ...
            def __new__(
                ...
                if tvars is not None:
                    ...
                    for base in bases:
                        ...
                        if isinstance(
                            base, GenericMeta
                        ) and base.__origin__ in (Generic, Protocol):
                        ...
                        if not tvarset <= gvarset:
                            raise TypeError(
                                ...
                                % (
                                    ...
                                    "Generic"
                                    if any(
                                        b.__origin__ is Generic for b in bases
                                    )
                                    else "Protocol",
                ...
                if (
                    ...
                    bases = (extra,) + bases
                bases = tuple(
                    _gorg(b) if isinstance(b, GenericMeta) else b
                    for b in bases
                )
                if any(
                    isinstance(b, GenericMeta) and b is not Generic
                    for b in bases
                ):
...
if sys.version_info >= (3, 9, 2):
    ...
    class _TypedDictMeta(type):
        ...
        def __new__(cls, name, bases, ns, total=True):
            ...
            tp_dict = super(_TypedDictMeta, cls).__new__(
                cls, name, (dict,), ns
            )
        ...
        def __getitem__(self, item):
            ...
            if self.__type__ is None:
                return cls(
                    typing._type_check(
                        item,
                        "{} accepts only a single type.".format(
                            cls.__name__[1:]
                        ),
                    ),
                    _root=True,
