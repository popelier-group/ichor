...
class Submodule(IndexObject, TraversableIterableObj):
    ...
    def _set_cache_(self, attr: str) -> None:
        if attr in ("path", "_url", "_branch_path"):
            ...
            self._branch_path = reader.get_value(
                self.k_head_option,
                ichor.git.Head.to_full_path(self.k_head_default),
            )
        elif attr == "_name":
    ...
    def _to_relative_path(
        cls, parent_repo: "Repo", path: PathLike
    ) -> PathLike:
    ...
    def add(
        ...
        br = ichor.git.Head(
            repo,
            ichor.git.Head.to_full_path(str(branch) or cls.k_head_default),
        )
        ...
        if url is None:
            if not has_module:
                raise ValueError(
                    "A URL was not given and a repository did not exist at %s"
                    % path
                )
            # END check url
            mrepo = sm.module()
            ...
            # clone new repo
            kwargs: Dict[str, Union[bool, int, str, Sequence[TBD]]] = {
                "n": no_checkout
            }
            ...
            if mrepo is not None and mrepo.head.commit.binsha != binsha:
                ...
                if mrepo.head.commit.binsha != self.NULL_BIN_SHA:
                    ...
                    if len(base_commit) == 0 or (
                        base_commit[0] is not None
                        and base_commit[0].hexsha == hexsha
                    ):
                        if force:
