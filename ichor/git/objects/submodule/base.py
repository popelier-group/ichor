...
from ichor.git.util import HIDE_WINDOWS_KNOWN_ERRORS
...
from ichor.git.types import Commit_ish, Literal, PathLike, TBD
...
class Submodule(IndexObject, TraversableIterableObj):
    ...
    def _set_cache_(self, attr: str) -> None:
        if attr in ('path', '_url', '_branch_path'):
            ...
            self._branch_path = reader.get_value(self.k_head_option, ichor.git.Head.to_full_path(self.k_head_default))
        elif attr == '_name':
    ...
    def _clone_repo(cls, repo: 'Repo', url: str, path: PathLike, name: str, **kwargs: Any) -> 'Repo':
        ...
        clone = ichor.git.Repo.clone_from(url, module_checkout_path, **kwargs)
        if cls._need_gitfile_submodules(repo.git):
    ...
    def add(cls, repo: 'Repo', name: str, path: PathLike, url: Union[str, None] = None,
        ...
        br = ichor.git.Head(repo, ichor.git.Head.to_full_path(str(branch) or cls.k_head_default))
        ...
        try:
            if configuration:
                try:
                    ...
                    nentry = ichor.git.IndexEntry(entry[:3] + (module_checkout_path,) + entry[4:])
    ...
    def module(self) -> 'Repo':
        ...
        try:
            repo = ichor.git.Repo(module_checkout_abspath)
            if repo != self.repo:
                return repo
    ...
    def branch_name(self) -> str:
        ...
        return ichor.git.Head(self.repo, self._branch_path).name
    ...
    def iter_items(cls, repo: 'Repo', parent_commit: Union[Commit_ish, str] = 'HEAD', *Args: Any, **kwargs: Any
        ...
        for sms in parser.sections():
            ...
            sm._branch_path = ichor.git.Head.to_full_path(b)
            sm._url = u
