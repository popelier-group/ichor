from typing import TYPE_CHECKING, Any, Callable, Iterator, Type, Union  # NOQA

from ichor.git.refs.symbolic import SymbolicReference, T_References
from ichor.git.types import _T, Commit_ish, PathLike  # NOQA
from ichor.git.util import IterableObj, LazyMixin
...
class Reference(SymbolicReference, LazyMixin, IterableObj):
    ...
    def __init__(
        self, repo: "Repo", path: PathLike, check_path: bool = True
    ) -> None:
        ...
        if check_path and not str(path).startswith(
            self._common_path_default + "/"
        ):
            raise ValueError(
