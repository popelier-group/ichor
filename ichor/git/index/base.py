...
class IndexFile(LazyMixin, git_diff.Diffable, Serializable):
    ...
    def __init__(
        self, repo: "Repo", file_path: Union[PathLike, None] = None
    ) -> None:
    ...
    @classmethod
    def from_tree(
        cls, repo: "Repo", *treeish: Treeish, **kwargs: Any
    ) -> "IndexFile":
        """Merge the given treeish revisions into a new index which is returned.
    ...
    def iter_blobs(
        self,
        predicate: Callable[[Tuple[StageType, Blob]], bool] = lambda t: True,
    ...
    def resolve_blobs(self, iter_blobs: Iterator[Blob]) -> "IndexFile":
        ...
        for blob in iter_blobs:
            ...
            if stage_null_key in self.entries:
                raise ValueError(
                    "Path %r already exists at stage 0" % str(blob.path)
                )
    ...
    def _process_diff_args(
        ...
        args: List[
            Union[str, "git_diff.Diffable", Type["git_diff.Diffable.Index"]]
        ],
    ) -> List[
        Union[str, "git_diff.Diffable", Type["git_diff.Diffable.Index"]]
    ]:
    ...
    def _store_path(
        self, filepath: PathLike, fprogress: Callable
    ) -> BaseIndexEntry:
            ...
            for path in paths:
                if osp.isabs(path):
                    ...
                    gitrelative_path = path[
                        len(str(self.repo.working_tree_dir)) + 1 :
                    ]
                else:
                    gitrelative_path = path
    ...
    def _items_to_rela_paths(
        ...
        items: Union[
            PathLike,
            Sequence[Union[PathLike, BaseIndexEntry, Blob, Submodule]],
        ],
    ) -> List[PathLike]:
