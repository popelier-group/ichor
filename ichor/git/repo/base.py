...
from ichor.git.exc import InvalidGitRepositoryError, NoSuchPathError, GitCommandError
from ichor.git.index import IndexFile
from ichor.git.objects import Submodule, RootModule, Commit
...
from ichor.git.util import Actor, finalize_process, decygpath, hex_to_bin, expand_path, remove_password_if_present
import os.path as osp

from ichor.git.repo.fun import rev_parse, is_git_dir, find_submodule_git_dir, touch, find_worktree_git_dir
import gc
import ichor.git.ext.gitdb

# typing ------------------------------------------------------

from ichor.git.types import TBD, PathLike, Lit_config_levels, Commit_ish, Tree_ish, assert_never
from typing import (Any, BinaryIO, Callable, Dict,
                    Iterator, List, Mapping, Optional, Sequence,
                    TextIO, Tuple, Type, Union,
                    NamedTuple, cast, TYPE_CHECKING)

from ichor.git.types import ConfigLevels_Tup, TypedDict
...
if TYPE_CHECKING:
    ...
    from ichor.git.remote import RemoteProgress
...
class Repo(object):
    ...
    def close(self) -> None:
        if self.git:
            ...
            if is_win:
                gc.collect()
            ichor.git.ext.gitdb.util.mman.collect()
            if is_win:
                gc.collect()
