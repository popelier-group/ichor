...
from ichor.git.types import Commit_ish
from ichor.git.util import bin_to_hex, decygpath, hex_to_bin
...
if TYPE_CHECKING:
    ...
    from ichor.git.objects import Blob, Commit, TagObject, Tree
    from ichor.git.refs.reference import Reference
    from ichor.git.refs.tag import Tag
        ...
        if obj is None:
            ...
            if start == 0:
                ...
                if token == "@":
                    ref = cast(
                        "Reference",
                        name_to_object(repo, rev[:start], return_ref=True),
                    )
                else:
        ...
        if start < lr and rev[start] == "{":
            ...
            if output_type == "commit":
                ...
                try:
                    ...
                    raise IndexError(
                        "Invalid revlog index: %i" % revlog_index
                    ) from e
