...
from ichor.git.compat import is_win
...
T_IterableObj = TypeVar(
    "T_IterableObj",
    bound=Union["IterableObj", "Has_id_attribute"],
    covariant=True,
)
...
from ichor.git.ext.gitdb.util import LazyMixin  # @UnusedImport
from ichor.git.ext.gitdb.util import LockedFD  # @UnusedImport
from ichor.git.ext.gitdb.util import bin_to_hex  # @UnusedImport
...
class Actor(object):
    ...
    def _main_actor(
        ...
        env_email: str,
        config_reader: Union[
            None, "GitConfigParser", "SectionConstraint"
        ] = None,
    ) -> "Actor":
        actor = Actor("", "")
        user_id = (
            None  # We use this to avoid multiple calls to getpass.getuser()
        )
...
class IterableList(List[T_IterableObj]):
    ...
    def __new__(
        cls, id_attr: str, prefix: str = ""
    ) -> "IterableList[IterableObj]":
