...
def natsort_keygen(key=None, alg=ns.DEFAULT):
    ...
    if alg & ns.LOCALEALPHA and locale.dumb_sort():
        alg |= NS_DUMB

    # Set some variables that will be passed to the factory functions
    if alg & ns.NUMAFTER:
        if alg & ns.LOCALEALPHA:
            sep = locale.null_string_locale_max
        else:
            sep = locale.null_string_max
        pre_sep = locale.null_string_max
    else:
        if alg & ns.LOCALEALPHA:
            sep = locale.null_string_locale
        else:
            sep = locale.null_string
        pre_sep = locale.null_string
