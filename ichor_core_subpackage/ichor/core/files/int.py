...
import numpy as np
from ichor.core.common.functools import (
    buildermethod,
    cached_property,
    classproperty,
)
...
class INT(DataFile, ReadFile, File):
    ...
    def __init__(self, path: Union[Path, str]):
        ...
        self.model: str = FileContents

        self.net_charge: float = FileContents
        self.basin_intergration_results: Dict[str, float] = FileContents
    ...
    def _read_file(self):
        ...
        with open(self.path, "r") as f:
            ...
            next(f)
            self.model = next(f).split(":")[-1].strip()
            ...
            self.basin_intergration_results = {}
            record = next(f).split()
            self.basin_intergration_results[record[0].strip()] = float(
                record[2]
            )
    ...
    def integration_error(self) -> float:
        ...
        return self.basin_intergration_results["L"]
