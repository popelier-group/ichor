...
class INT(DataFile, ReadFile):
    ...
    def local_spherical_multipoles(self, C_matrix: Optional[np.ndarray] = None) -> Dict[str, float]:
        ...
        if C_matrix:
            ...
            if self.parent is not None:
                C = self.parent[self.atom_name].C()
            else:
                raise ValueError(f"Cannot rotate multipoles as self.parent is {self.parent}. We need the whole molecular geometry to do multipole rotations.")
