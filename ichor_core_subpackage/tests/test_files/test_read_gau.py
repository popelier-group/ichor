...
example_dir = get_cwd(__file__) / ".." / ".." / ".." / "example_files" / "example_gaus"
...
def _assert_atomic_forces(
    ...
    if gau_file_forces and reference_file_forces:
        for reference_atom_name, reference_atom_forces in reference_file_forces.items():
            assert gau_file_forces[reference_atom_name].x == pytest.approx(
                reference_atom_forces.x
            )
            assert gau_file_forces[reference_atom_name].y == pytest.approx(
                reference_atom_forces.y
            )
            assert gau_file_forces[reference_atom_name].z == pytest.approx(
                reference_atom_forces.z
            )
...
def test_glycine_gau_file_with_forces():
    ...
    reference_atoms = Atoms(
        [
            Atom("C", -0.91371, 1.58371, 0.27699),
