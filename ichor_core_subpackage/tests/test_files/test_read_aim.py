...
def _assert_aim_atoms(
    aim_file_atoms: Dict[str, AimAtom], reference_aim_atoms: Dict[str, AimAtom]
):

    """Asserts that AIM atoms (containing info for an atom in aimall
    such as time taken, etc.) is equal to reference AIM atoms"""
    ...
    for reference_aim_atom_name, reference_aim_atom in reference_aim_atoms.items():
        assert (
            aim_file_atoms[reference_aim_atom_name].atom_name
            == reference_aim_atom.atom_name
        )
        assert (
            aim_file_atoms[reference_aim_atom_name].inp_file
            == reference_aim_atom.inp_file
        )
        assert (
            aim_file_atoms[reference_aim_atom_name].int_file
            == reference_aim_atom.int_file
        )
        assert (
            aim_file_atoms[reference_aim_atom_name].time_taken
            == reference_aim_atom.time_taken
        )
        assert aim_file_atoms[
            reference_aim_atom_name
        ].integration_error == pytest.approx(reference_aim_atom.integration_error)
