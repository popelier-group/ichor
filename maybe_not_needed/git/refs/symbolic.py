...
from ichor.git.objects import Object
...
if TYPE_CHECKING:
    ...
    from ichor.git.objects.commit import Actor
    from ichor.git.refs import Head, Reference, RemoteReference, TagReference
    from ichor.git.refs.log import RefLogEntry
...
class SymbolicReference(object):
    ...
    def _iter_packed_refs(cls, repo: "Repo") -> Iterator[Tuple[str, str]]:
        ...
        try:
            with open(
                cls._get_packed_refs_path(repo), "rt", encoding="UTF-8"
            ) as fp:
                for line in fp:
                    line = line.strip()
                    if not line:
    ...
    def set_reference(
        ...
        if isinstance(ref, SymbolicReference):
            ...
            try:
                ...
                raise ValueError(
                    "Could not extract object from %s" % ref
                ) from e
            # END end try string
        else:
        ...
        ok = True
        try:
            fd._write_file(write_value.encode("ascii") + b"\n")
            lfd.commit()
            ok = True
        finally:
            if not ok:
    ...
    @classmethod
    def to_full_path(
        cls, path: Union[PathLike, "SymbolicReference"]
    ) -> PathLike:
        """
            ...
            try:
                with open(pack_file_path, "rb") as reader:
                    ...
                    for line_bytes in reader:
                        ...
                        # we drop it as well
                        if (
                            line.startswith("#") or full_ref_path != line_ref
                        ) and (
                ...
                if made_change:
                    ...
                    with open(pack_file_path, "wb") as fd:
                        fd.writelines(
                            line.encode(defenc) for line in new_lines
                        )
    ...
    def rename(
        self, new_path: PathLike, force: bool = False
    ) -> "SymbolicReference":
    ...
    def is_remote(self) -> bool:
        ...
        return str(self.path).startswith(
            self._remote_common_path_default + "/"
        )
