...
def calculate_rmse(
    ...
    for model in models:
        ...
        # keys: integers corresponding to number of training points, values:
        # a nested dictionary which first layer being the atom name and second being property name
...
def make_rmse_chart_settings(local_kwargs: dict):
    ...
    into the `write_to_excel` function. Then, this function constructs dictionaries with
    parameter values to be passed to xlsx writer to configure graph settings.
...
def calculate_error(data: np.array, error_type: str):
    ...
    :param data: A one dimensional numpy array which contains the (absolute)
        difference between true and predicted values for a property.
...
def write_to_excel(
    ...
    error_type: Literal["mae", "rmse"] = "rmse",
    ...
    """
    Writes out relevant information which is used to write rmse to an excel file.
    It will make a separate sheet for training set and a final sheet containing MAE/RMSE errors.
    ...
    :param error_type: The type of error to calculate. Either Mean Absolute Error (MAE)
        or Root Mean Squared Error (RMSE). Default is RMSE ("rmse").
    :param only_every_nth_model: Only write out every `nth` model to the excel file,
        where `n` is an integer. This is useful if you have a very large number of models and only
    ...
    # TODO: move this is the calculate_rmse function where you can do this before making predictions,
    # right now it makes the predictions and then deletes.
    # leave every nth model, delete the other ones.
    # This can come in useful if you have a lot of models and do not want to go though each one
    ...
    with pd.ExcelWriter(output_name) as writer:
        ...
        # make a dictionary of dictionaries which is used to write out the final
        # sheet containing MAE errors for each n training point model
        ...
        for ntrain, true, predicted in zip(
            ...
            for type_ in true.keys():
                ...
                for atom in true[type_].keys():
                    ...
                    all_errors_dict[f"{atom}_{type_} (kJ mol-1)"][
                        ntrain
                    ] = error_estimator

                # after looping thorugh all atoms, we can sum up all the errors
                # to make a total error column for one property
            ...
            # get the number of rows. Since excel starts counting from 1,
            # this number will be the next empty row where we can write RMSE
            n_rows = df.shape[0]
        ...
        writer.sheets[error_type].insert_chart(
            f"{num2col(df.shape[1]+3)}19", rmse_plot2
        )
