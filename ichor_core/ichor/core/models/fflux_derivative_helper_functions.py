...
def EqC12(iatm_idx, omega, diff, atoms_instance, system_alf):
    ...
    for j in range(3):
        res[0] = res[0] + (y_v[j] * dyj[0, j])
        res[1] = res[1] + (y_v[j] * dyj[1, j])
        res[2] = res[2] + (y_v[j] * dyj[2, j])
...
def dZj_da(iatm_idx, jatm_idx, omega, j, atoms_instance, system_alf):
    ...
    res = np.zeros(3)
    # zeta = np.zeros(3)
    C1_vec = C_1(iatm_idx, atoms_instance, system_alf)
    C2_vec = C_2(iatm_idx, atoms_instance, system_alf)
    C3_vec = C_3(iatm_idx, atoms_instance, system_alf)
    if j == 1:
        Cjk = C1_vec
        ...
        dCjk = dC3k_da(C1_vec, C2_vec, dC1k, dC2k)
