...
def submit_overfitting_for_folders(
    model_folders: List[Path], ncores: int
    ...
    :param ncores: Number of cores each job asks for, which is also the number of models
        it checks at once.
...
def overfitting_submission_notes(ncores: int) -> List[str]:
    ...
    return [
        f"One job per model folder, each checking {ncores} models at once and writing "
        f"its report to {ANALYSIS_SUBFOLDER}/{OVERFITTING_CSV_NAME} inside that "
        "folder, next to the rest of that batch's analysis.",
